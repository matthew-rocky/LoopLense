...
export default async function DashboardPage() {
  try {
    const [summary, loops] = await Promise.all([getSummary(), getLoops({ limit: 600 })]);
      ...
      <PageShell>
        <section className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
          <Card className="relative overflow-hidden p-7">
            ...
            <p className="mt-4 max-w-3xl text-base leading-7 text-[var(--muted)]">
              LoopLens ranks circular charity funding patterns, enriches them with organization names, and keeps every answer grounded in loaded records.
            </p>
          ...
          <Card className="flex flex-col justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--muted)]">Not a finding of wrongdoing.</p>
              </div>
            </div>
            <p className="mt-5 text-sm leading-6 text-[var(--muted)]">
              Scores and labels are review-priority indicators. Reviewers should inspect source records and context before taking action.
            </p>
          </Card>
        </section>
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold">Top Review-Priority Loops</h2>
            <Link href="/loops" className="text-sm font-semibold text-[var(--accent)]">View all</Link>
          </div>
          <div className="grid gap-3 2xl:grid-cols-2">
            {summary.top_high_priority_loops.slice(0, 8).map((row) => (
              <Link key={loopId(row)} href={`/loops/${encodeURIComponent(loopId(row))}`} className="group flex items-center justify-between gap-4 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] p-4 transition hover:-translate-y-0.5 hover:bg-[var(--surface)]">
                ...
                <div className="flex shrink-0 items-center gap-3">
                  <Badge label={row.review_label} />
                  <span className="text-sm text-[var(--muted)]">{score(row.review_score)}</span>
                </div>
              </Link>
            ))}
          </div>
...
function Action({ href, icon: Icon, label }: { href: string; icon: ComponentType<{ size?: number }>; label: string }) {
  return (
    <Link href={href} className="inline-flex items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-2 text-sm font-semibold transition hover:-translate-y-0.5 hover:bg-[var(--surface)]">
      <Icon size={16} />
      {label}
    </Link>
  );
