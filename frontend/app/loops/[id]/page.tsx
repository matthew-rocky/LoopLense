...
export default async function LoopDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
            ...
            <table className="w-full min-w-[980px] text-left text-sm">
              <thead className="sticky top-0 bg-[var(--surface-strong)] text-xs uppercase text-[var(--muted)]">
                <tr>
                  ...
                  <th className="px-4 py-3">Sends To</th>
                  <th className="px-4 py-3">Receives From</th>
                </tr>
              </thead>
    ...
    return (
      <PageShell>
        <EmptyState icon={Search} title="Loop unavailable" description="The loop could not be loaded. Confirm the backend is running and the loop ID exists." />
      </PageShell>
    );
  }
}
